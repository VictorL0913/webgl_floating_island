...
// Main JavaScript file - COMPLETE FIX FOR SHADOWS + Windmill (front side: 0,0,5)
...
// Windmill data with corrected dimensions
let windmill = {
    baseObj: null,           // base under the pole
    poleObj: null,           // pole object
    hubObj: null,            // small cube hub at pole tip
    blades: [],             // array of { obj, initAngle }
    rotation: 0,
    poleRotation: 0,        // rotation for pole direction
    position: [0, 9],       // (x, z) - front side: 0, 5
    poleHeight: 4,        // Pole height
    poleWidth: 0.1,        // Thinner pole
    bladeLength: 1.2,       // Keep original blade length
    bladeThickness: 0.08,   // Thin blades
    bladeWidth: 0.1,        // Width of blades
    baseSize: 1.0,          // Base size under the pole
    baseHeight: 1,        // Base height 
    hubSize: 0.2           // Size of small cube hub
};

// Object tracking
let allObjects = []; // All objects that should cast shadows
...
const fragmentShaderSource = `#version 300 es
    ...
    void main() {
        ...
        vec3 result = ambient + (1.0 - shadow * 0.7) * intensity * attenuation * (diffuse + specular);
...
function createIsland() {
    ...
    for (let layer = 0; layer < layers; layer++) {
        ...
        // Add to allObjects for shadow casting
        allObjects.push({
            vao: cube.vao,
            count: cube.count,
...
// Create Windmill: base + pole + hub + 4 blades
function createWindmill(position) {
    const [x, z] = position;
    const baseSize = windmill.baseSize;
    const baseHeight = windmill.baseHeight;
    const poleHeight = windmill.poleHeight;
    const poleWidth = windmill.poleWidth;
    const bladeLength = windmill.bladeLength;
    const bladeThickness = windmill.bladeThickness;
    const bladeWidth = windmill.bladeWidth;
    const hubSize = windmill.hubSize;

    // Create base (larger block under the pole)
    const baseVAO = createCube([0, 0, 0], [baseSize, baseHeight, baseSize], [0.6, 0.5, 0.4]);
    const baseObj = {
        vao: baseVAO.vao,
        count: baseVAO.count,
        modelMatrix: mat4.create()
    };
    // Position base to touch ground (-0.4 is ground level)
    const baseY = -0.4 + (baseHeight / 2);
    mat4.identity(baseObj.modelMatrix);
    mat4.translate(baseObj.modelMatrix, baseObj.modelMatrix, [x, baseY, z]);

    allObjects.push(baseObj);
    windmill.baseObj = baseObj;

    // Create pole (thin tower on top of base) - SHORTER
    const poleVAO = createCube([0, 0, 0], [poleWidth, poleHeight, poleWidth], [0.85, 0.85, 0.85]);
    const poleObj = {
        vao: poleVAO.vao,
        count: poleVAO.count,
        modelMatrix: mat4.create()
    };
    // Position pole on top of base
    const poleY = baseY + (baseHeight / 2) + (poleHeight / 2);
    mat4.identity(poleObj.modelMatrix);
    mat4.translate(poleObj.modelMatrix, poleObj.modelMatrix, [x, poleY, z]);

    allObjects.push(poleObj);
    windmill.poleObj = poleObj;

    // Create hub (small cube at tip of pole where blades connect)
    const hubVAO = createCube([0, 0, 0], [hubSize, hubSize, hubSize], [0.7, 0.7, 0.7]);
    const hubObj = {
        vao: hubVAO.vao,
        count: hubVAO.count,
        modelMatrix: mat4.create()
    };
    // Position hub at top of pole (which is now lower)
    const hubY = poleY + (poleHeight / 2) + (hubSize / 2);
    mat4.identity(hubObj.modelMatrix);
    mat4.translate(hubObj.modelMatrix, hubObj.modelMatrix, [x, hubY, z]);

    allObjects.push(hubObj);
    windmill.hubObj = hubObj;

    // Add windmill collision (using base size)
    collisionObjects.push({
        type: 'windmill',
        position: [x, 0, z],
        radius: baseSize * 0.8,
    ...
    // Create 4 blades (each a cube centered at origin)
    windmill.blades = [];
    for (let i = 0; i < 4; i++) {
        const initAngle = i * (Math.PI / 2);
        // blade geometry: long along X axis (for rotation around Z)
        const bladeVAO = createCube([0, 0, 0], [bladeLength, bladeThickness, bladeWidth], [0.95, 0.95, 0.95]);
        const bladeObj = {
            vao: bladeVAO.vao,
            count: bladeVAO.count,
            modelMatrix: mat4.create()
        };
        // initial modelMatrix (will be updated every frame)
        mat4.identity(bladeObj.modelMatrix);
        allObjects.push(bladeObj);
        windmill.blades.push({ obj: bladeObj, initAngle: initAngle });
    }
}
...
function handleCollisions(newX, newZ) {
    ...
    for (const obj of collisionObjects) {
        ...
        if (distance < minDistance) {
            ...
            // Prevent car from getting stuck by ensuring at least minDistance away
...
function createSceneObjects() {
    ...
    // Create windmill at front side (0, 5)
    createWindmill([windmill.position[0], windmill.position[1]]);
...
function renderShadowPass() {
    ...
    mat4.ortho(lightProjection, -20, 20, -20, 20, 1, 100);
...
function renderMainPass() {
    ...
    gl.uniform3fv(lightColorLoc, [1.5, 1.5, 1.3]); // light color intensity
    gl.uniform3fv(ambientLoc, [0.3, 0.3, 0.3]); // Lower ambient
...
function updateCamera() {
    ...
    // R and F keys to rotate windmill pole
    if (keys['KeyR']) {
        windmill.poleRotation += 0.02; // Rotate clockwise
    }
    if (keys['KeyF']) {
        windmill.poleRotation -= 0.02; // Rotate counter-clockwise
    }
...
function updateWindmill() {
    ...
    windmill.rotation += 0.03;
    
    // Compute positions
    const baseY = -0.4 + (windmill.baseHeight / 2);
    const baseTopY = baseY + (windmill.baseHeight / 2);
    const poleY = baseTopY + (windmill.baseHeight / 2);
    const hubY = poleY + (windmill.poleHeight / 2) + (windmill.hubSize / 2) + 1.5;
    const hubOffsetZ = windmill.poleWidth / 2 + windmill.hubSize / 2; // front offset from pole
    const bladeOffsetExtra = 0;
    
    // Update base modelMatrix
    if (windmill.baseObj) {
        mat4.identity(windmill.baseObj.modelMatrix);
        mat4.translate(windmill.baseObj.modelMatrix, windmill.baseObj.modelMatrix, 
                      [windmill.position[0], baseY, windmill.position[1]]);
    }
    
    // Update pole modelMatrix with rotation
    if (windmill.poleObj) {
        mat4.identity(windmill.poleObj.modelMatrix);
        mat4.translate(windmill.poleObj.modelMatrix, windmill.poleObj.modelMatrix, 
                      [windmill.position[0], poleY, windmill.position[1]]);
        mat4.rotateY(windmill.poleObj.modelMatrix, windmill.poleObj.modelMatrix, windmill.poleRotation);
    }
    
    // Update hub (on top + front of pole)
    if (windmill.hubObj) {
        mat4.identity(windmill.hubObj.modelMatrix);
        mat4.translate(windmill.hubObj.modelMatrix, windmill.hubObj.modelMatrix,
                      [windmill.position[0], hubY, windmill.position[1] + hubOffsetZ]);
        mat4.rotateY(windmill.hubObj.modelMatrix, windmill.hubObj.modelMatrix, windmill.poleRotation);
    }
    
    // Update each blade's modelMatrix
    // Blades attach to the SIDE faces of the hub cube
    windmill.blades.forEach(b => {
        const bladeObj = b.obj;
        const angle = windmill.rotation + b.initAngle;

        mat4.identity(bladeObj.modelMatrix);
        
        // Move to hub center (already offset from pole)
        mat4.translate(bladeObj.modelMatrix, bladeObj.modelMatrix, [
            windmill.position[0], 
            hubY, 
            windmill.position[1] + hubOffsetZ
        ]);

        // Apply pole rotation (so blades follow pole direction)
        mat4.rotateY(bladeObj.modelMatrix, bladeObj.modelMatrix, windmill.poleRotation);

        // Rotate around Z axis for blade spinning
        mat4.rotateZ(bladeObj.modelMatrix, bladeObj.modelMatrix, angle);
        const bladeOffsetZ = windmill.hubSize / 2 + windmill.bladeLength / 2 - 0.42;
        mat4.translate(bladeObj.modelMatrix, bladeObj.modelMatrix, [0, 0, bladeOffsetZ]);
    });
}

// Main render loop
function render() {
    // Update camera and car first
    updateCamera();
    updateCar();
    ...
    // Car drawn after main pass so car's modelMatrix is applied separately
    renderCar();
